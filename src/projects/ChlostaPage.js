...
class ChlostaPage extends Component {
  componentDidMount() {
    document.title = 'Chlosta';
    window.scrollTo(0, 0);
  }
          ...
          <Row>
            <Col>
              <Carousel activeItem={1} length={4} showControls={true} showIndicators={true} className="z-depth-1 mt-5">
                <CarouselInner>{this.renderCarousel(data.images.Chlosta)}</CarouselInner>
              </Carousel>
            </Col>
          </Row>
        </Container>
        <Container>
          <Row className="mt-5">{this.renderFlip(data.tools.Chlosta)}</Row>
        </Container>
      </div>
    );
  }
